...
async function Deploy() {
    ...
    const cw20CodeId = await storeCode(
      ...
      result = deploy.logs[0].events[0].attributes[3].value;

      console.log(chalk.green("Done!"), `${chalk.blue("Attributes")}=${result}`);
...
async function instantiateContract(
    terra,
    deployer,
    admin, // leave this emtpy then contract is not migratable
    codeId,
    instantiateMsg
  ) {
